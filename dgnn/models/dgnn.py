...
from dgnn.models.modules.layers import *
from dgnn.models.modules.memory import *
...
class DGNN(torch.nn.Module):
    ...
    def __init__(self, dim_node, dim_edge, sample_param, memory_param, gnn_param, train_param):
        ...
        for l in range(gnn_param['layer']):
            for h in range(sample_param['history']):
                if l == 0:
                    dim_node = self.dim_node_input
                else:
                    dim_node = gnn_param['dim_out']

                key = 'l' + str(l) + 'h' + str(h)
                self.layers[key] = TransfomerAttentionLayer(dim_node,
                                                            dim_edge,
                                                            gnn_param['dim_time'],
                                                            gnn_param['dim_out'],
                                                            gnn_param['att_head'],
