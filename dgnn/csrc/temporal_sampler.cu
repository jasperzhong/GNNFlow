...
//     Copy(root_nodes_dst, all_nodes.data(), all_nodes.size() *
//     sizeof(NIDType)); Copy(root_timestamps_dst, all_timestamps.data(),
...
//     std::size_t offset1 = max_sampled_nodes * sizeof(NIDType);
//     std::size_t offset2 = offset1 + max_sampled_nodes * sizeof(EIDType);
//     std::size_t offset3 = offset2 + max_sampled_nodes *
//     sizeof(TimestampType); std::size_t offset4 = offset3 + max_sampled_nodes
//     * sizeof(TimestampType);

//     std::size_t total_output_size = offset4 + num_root_nodes *
//     sizeof(uint32_t); total_output_size_list[snapshot] = total_output_size;
...
//     } else if (sampling_policy_ == SamplingPolicy::kSamplingPolicyUniform) {
//       int offset_per_thread =
//           shared_memory_size_ / sizeof(SamplingRange) /
//           num_threads_per_block;
...
//     auto d_src_nodes =
//     reinterpret_cast<NIDType*>(gpu_output_buffer_[snapshot]); auto d_eids =
//         reinterpret_cast<EIDType*>(gpu_output_buffer_[snapshot] + offset1);
...
//     NIDType* src_nodes = reinterpret_cast<NIDType*>(cpu_buffer_[snapshot]);
//     EIDType* eids = reinterpret_cast<EIDType*>(cpu_buffer_[snapshot] +
//     offset1); TimestampType* timestamps =
//         reinterpret_cast<TimestampType*>(cpu_buffer_[snapshot] + offset2);
//     TimestampType* delta_timestamps =
//         reinterpret_cast<TimestampType*>(cpu_buffer_[snapshot] + offset3);
//     uint32_t* num_sampled =
...
//                               sizeof(NIDType) * num_sampled_nodes,
//                               cudaMemcpyDeviceToHost, streams_[snapshot]));
//     CUDA_CALL(cudaMemcpyAsync(eids, d_eids, sizeof(EIDType) *
//     num_sampled_nodes,
//                               cudaMemcpyDeviceToHost, streams_[snapshot]));
//     CUDA_CALL(cudaMemcpyAsync(timestamps, d_timestamps,
//                               sizeof(TimestampType) * num_sampled_nodes,
...
//     if (layer == 0) {
//       results.push_back(
//           SampleLayer(layer, RootInputToSamplingResult(dst_nodes,
//           timestamps)));
//     } else {
