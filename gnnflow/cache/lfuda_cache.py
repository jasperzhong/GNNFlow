...
class LFUDACache(Cache):
    ...
    def resize(self, new_num_nodes: int, new_num_edges: int):
        ...
        if self.dim_edge_feat != 0:
            self.cache_edge_count.resize_(self.new_num_edges)
    ...
    def update_node_cache(self, cached_node_index: torch.Tensor,
        ...
        removing_node_id = torch.topk(
            self.cache_node_count[self.cache_node_flag], k=num_node_to_cache, largest=False).indices
        assert len(removing_node_id) == len(
    ...
    def update_edge_cache(self, cached_edge_index: torch.Tensor,
        ...
        removing_edge_id = torch.topk(
            self.cache_edge_count[self.cache_edge_flag], k=num_edge_to_cache, largest=False).indices
        assert len(removing_edge_id) == len(
