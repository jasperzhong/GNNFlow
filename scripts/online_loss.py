...
def train(args, epoch, path_saver, df, rand_sampler, val_df, val_rand_sampler,
...
# update loss in one epoch
train(args, 1, path_saver, phase1_train_df, rand_sampler,
      phase1_val_df, val_rand_sampler, sampler, model, None,
...
for i, (target_nodes, ts, eid) in enumerate(get_batch(phase2_df, None, incremental_step)):
    ...
    if i % args.retrain == 0 and i != 0:
        ...
        train(args, args.epoch, path_saver, phase2_train_df, rand_sampler,
