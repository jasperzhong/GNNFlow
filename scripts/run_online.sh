...
if [[ $NPROC_PER_NODE -gt 1 ]] ; then
    cmd="torchrun \
        ...
        --replay-ratio $REPLAY --phase1-ratio $PHASE1_RATIO"
else
...
rm -rf /dev/shm/rmm_*
echo $cmd
OMP_NUM_THREADS=8 exec $cmd > no-retrain-30%online-$MODEL-$DATA-$CACHE-$EDGE_CACHE_RATIO-$NODE_CACHE_RATIO-$NPROC_PER_NODE-$REPLAY-$PHASE1_RATIO.log 2>&1
