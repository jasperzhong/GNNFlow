...
echo $cmd
LOGLEVEL=DEBUG CUDA_LAUNCH_BLOCKING=1 OMP_NUM_THREADS=8 RANK=$RANK LOCAL_RANK=0 WORLD_SIZE=$NNODES LOCAL_WORLD_SIZE=$NPROC_PER_NODE MASTER_ADDR=$HOST_NODE_ADDR MASTER_PORT=$HOST_NODE_PORT gdb -ex run --args $cmd
