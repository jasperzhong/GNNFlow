...
class TestTemporalSampler(unittest.TestCase):
    def test_sampler_layer(self):
        ...
        for block in blocks:
            self.assertEqual(block.srcdata['ID'].tolist(), [0, 1, 2, 2, 1, 2, 1, 2, 1])
            self.assertEqual(block.srcdata['ts'].tolist(), [1.5, 1.5, 1.5, 1, 0, 1, 0, 1, 0])
            self.assertEqual(block.edata['dt'].tolist(), [0.5, 1.5, 0.5, 1.5, 0.5, 1.5])
            self.assertEqual(block.edata['ID'].tolist(), [1, 0, 4, 3, 7, 6])

        print("Test sampler_layer passed")
